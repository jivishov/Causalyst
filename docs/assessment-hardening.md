...
The review was organized as [private keys](https://github.com/jivishov/Causalyst/pull/1), [enrollment](https://github.com/jivishov/Causalyst/pull/2), [dependencies](https://github.com/jivishov/Causalyst/pull/3), [evidence/grading](https://github.com/jivishov/Causalyst/pull/4), and [live voice and browser hardening](https://github.com/jivishov/Causalyst/pull/5). [PR #6](https://github.com/jivishov/Causalyst/pull/6) consolidates those changes, database types, real Supabase integration and the final fidelity refinements for `main`. Deploy the completed change set using the migration order below.

The original five-PR stack passed 332 distinct unit tests on Node 22.23.2, replayed 27 migrations with five SQL suites in PGlite, and passed three Chromium browser tests. The production build, Worker dry run, bundle and secret scans passed; the audited dependency tree reported zero known vulnerabilities. Initial JavaScript measured 453,408 bytes / 132,630 gzip bytes. The browser executable was supplied locally because the standard browser download failed in this environment; CI installs Playwright's Chromium. Native PostgreSQL 17 migration replay and concurrent enrollment, submission, reservation and cleanup checks also passed in [GitHub CI](https://github.com/jivishov/Causalyst/actions/runs/35694434878). Hosted-provider behavior has separate gates below. The empty teacher-evaluation template correctly refuses to pass. Shared-package tests exclude compiled `dist` copies so local reruns and clean CI count the same source tests.

The database-type follow-up passed both jobs in [CI run 35738661993](https://github.com/jivishov/Causalyst/actions/runs/35738661993) at commit `02193547747c5b323b09fc490e2f60d48e614829`: 341 unit tests, three Chromium tests, all 27 migrations, the SQL suites and concurrent races, plus five integration tests against real Supabase services. Regenerating database types produced no drift. Type checking includes the integration sources and compile-time database contracts. Build, bundle, dependency audit and secret checks also passed; no production configuration or data was changed.

## Review map

The final review found and corrected these additional gaps:

| Gap | Refinement and evidence |
| --- | --- |
| Push-triggered deployments could race each other and precede database migrations | Deployment is manual after an explicit migration/staging acknowledgement; both validation jobs run first, then Worker and frontend deploy from the same commit. The acknowledgement does not inspect the hosted database. |
| Live evidence could grow beyond a storage value's limit, and failed hangup could interrupt finalization or cleanup | Bound combined transcript text to 64 KiB UTF-8 and 500 turns; reject conflicting completions; persist only evidence changes; acknowledge sealing only after storage succeeds; retry provider closure independently while preserving sealed evidence. Storage/provider failure tests cover these paths. |
| A provider cancellation failure stopped later cleanup and local recovery | Run database recovery before provider operations, expire jobs independently, persist pending cancellation and retry without restarting generation. A Supabase integration test injects a provider outage and checks subsequent cancellation recovery. |
| Evaluation input could inflate confidence through truthy review declarations or repeated cases | Require literal `teacherReviewed: true`, unique case IDs, identified dataset/model/policy and valid thresholds. Regression tests reject malformed declarations, duplicate cases and numeric scoring-cap IDs. |
...
The separate `supabase` CI job starts real Auth, Storage and PostgREST containers, replays migrations against Supabase-owned schemas, generates and compares the committed types, and runs six integration tests. Synthetic accounts sign in through Auth; anonymous, student and unrelated-user clients must be denied private keys, snapshots and direct Storage access. Worker helpers exercise upload ownership, immutable bytes, submission manifests and complete 1,201-row CSV export with an actual 73-row PostgREST limit. A mocked provider outage tests database cancellation/retry state. The SQL role/transaction suites and concurrent races also run against that stack. Teardown removes its local data even when tests fail.
...
- Live collection stores at most 64 KiB of combined UTF-8 transcript text and 500 turns. Exceeding the bound fails closed rather than grading a truncated prefix. Provider hangup retries run independently of the sealed transcript, which remains available until its one-day expiry. A pending call identifier can remain after transcript deletion solely for closure retries. Tests simulate storage/provider failures; they do not certify the deployed Cloudflare integration. The storage bound is deliberately below the [SQLite Durable Object value limit](https://developers.cloudflare.com/durable-objects/platform/limits/).
...
| Pre-upgrade draft artifacts with no verified hash | Private `app_private.legacy_artifact_retention_holds` records the IDs at cutover and clears their Storage cleanup deadline. SQL cleanup claims refuse held IDs even if a deadline is later set. New reservations have no hold and keep the seven-day policy. Include this private table and the Storage objects in backups; release holds only after an explicit retention decision. |
...
Cleanup runs every ten minutes. Database recovery runs before external provider work. Inspect failed scheduled invocations, `attempt_artifacts.cleanup_error` / `cleanup_attempts >= 10`, and expired simulation jobs with `provider_status='cancellation_pending'`; resolve the cause before resetting retry counters. Pending cancellations are retried without restarting generation, and each provider cleanup request has a five-second timeout. Provider deletion and Storage deletion are idempotent external operations, followed by a transactional cleanup record and audit entry. A provider request whose file handle never reaches the database relies on provider expiry; the implementation cannot enumerate unknown provider objects from an absent handle.
...
1. Review the combined change set and run both validation jobs (`checks` and `supabase`) before merging. Configure required checks in branch protection for future changes; repository settings remain a separate administrator action. Use a disposable PostgreSQL database for `test:db` because its bootstrap creates fixture roles and schemas. A push to `main` runs validation and does not deploy.
2. Verify a private, restorable **database and Storage-object backup** before changing the hosted project; on a plan without managed backups, arrange and verify a separate export rather than assuming one exists. Include `app_private.legacy_artifact_retention_holds` in the database backup after cutover. Back up staging and replay all migrations against a representative copy without student identifiers. Confirm Auth/Storage schema compatibility, Worker service-role grants and denied direct `anon`/`authenticated` access. The local PGlite run skips only the `pgcrypto` extension declaration; CI uses native PostgreSQL 17 and two separate connections for race tests. The additional disposable Supabase job checks real service integration, but it does not certify the hosted project's configuration or historical data.
3. Schedule an assessment maintenance window and stop assessment writes until the matching Worker is live. The new manifest-aware submission function and snapshot requirement are incompatible with the old Worker submission path. Apply the pending SQL migrations in filename order, **including the final legacy-retention hold migration**. That migration records previously unhashed draft objects and cancels their new cleanup deadlines before the new cron starts. Verify hold counts and claims, then manually run **Deploy Worker** on `main` with the migration/staging acknowledgement set to true. It reruns full validation, deploys the matching Worker with the `REALTIME_SESSIONS` Durable Object migration and cron, and only then calls the frontend deployment at the same commit. The acknowledgement is a declaration by the release operator, not a database inspection. Do not roll the Worker back alone against this schema.
