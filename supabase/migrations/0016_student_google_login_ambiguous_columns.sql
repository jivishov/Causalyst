...
begin
  ...
  select class_row.id, class_row.code, class_row.name
  into v_class
  ...
  select
    access_row.id,
    access_row.class_id,
    access_row.student_label,
    access_row.pin_hash,
    access_row.claimed_by,
    access_row.claimed_at,
    access_row.roster_student_id
  into v_access
  ...
  select
    roster_row.id,
    roster_row.class_id,
    roster_row.display_name,
    roster_row.email,
    roster_row.claimed_by,
    roster_row.claimed_at
  into v_roster
  ...
  select profile_row.id, profile_row.role, profile_row.display_name, profile_row.email
  ...
  if v_previous_user_id is not null then
    ...
    set
      ...
      updated_at = v_now
    where attempt.student_id = v_previous_user_id
      and attempt.assignment_id = any(v_assignment_ids);
  ...
  on conflict on constraint class_memberships_class_id_student_id_key do update
    set display_name = excluded.display_name,
        roster_student_id = excluded.roster_student_id;

  update public.student_access_codes as access_code
  set
    claimed_by = p_user_id,
    claimed_at = coalesce(access_code.claimed_at, v_now)
  where access_code.id = v_access.id;

  update public.roster_students as roster_student
  set
    claimed_by = p_user_id,
    claimed_at = coalesce(roster_student.claimed_at, v_now),
    updated_at = v_now
  where roster_student.id = v_roster.id;
