...
export async function runRetention(db: AppDatabaseClient, env: Env): Promise<void> {
  ...
  for (const artifact of data ?? []) {
    ...
    try {
      if (artifact.openai_file_id && artifact.provider_cleanup_at && artifact.provider_cleanup_at <= now) {
        try { await client.files.delete(artifact.openai_file_id); } catch (error) {
  ...
  const { data: expired, error: expiredError } = await db.from("simulation_generation_jobs").select("id,provider,provider_response_id")
    .in("status", ["queued", "in_progress"]).lte("expires_at", now).limit(25);
  ...
  for (const job of expired ?? []) {
    if (job.provider === "openai" && job.provider_response_id) {
      try { await client.responses.cancel(job.provider_response_id); } catch (error) {
        if (!(error && typeof error === "object" && "status" in error && [400, 404, 409].includes(Number(error.status)))) throw error;
      }
    }
    const { error } = await db.from("simulation_generation_jobs").update({ status: "expired", completed_at: now, error_message: "Generation expired." })
      .eq("id", job.id).in("status", ["queued", "in_progress"]);
